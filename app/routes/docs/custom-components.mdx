...
Different from native elements, custom components has an internal state controlled by a state management, it means that they will be updated when the value changes, however native elements are not, because you can use a reference to the element to update its value.
...
### Let's see some examples of custom components.
 ...
 - Selects, multi-selects components - are a component that allows you to select a single or multiple values.
...
Usually custom components doesn't work with reference, so we need to use a wrapper to connect the component with the form. So you can use the `register` function to register the custom component.
<br/>
